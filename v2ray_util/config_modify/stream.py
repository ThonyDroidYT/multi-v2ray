...
def modify(group=None, sType=None):
    ...
    if group == None:
        ...
        if sType != None:
            ...
            print(_("protocolo modificado con éxito"))
            return
        ...
        print("")
        choice = input(_("por favor seleccione nuevo protocolo: "))

        if not choice.isdecimal():
            print(_("Por favor ingrese un número!"))
        else:
            choice = int(choice)
            if choice > 0 and choice <= len(sm.stream_type):
                if sm.stream_type[choice - 1][1] in ("MTProto", "Shadowsocks") and group.tls == 'tls':
                    print(_("{} MTProto/Shadowsocks ¡No es compatible con HTTPS, TLS cerrado con éxito!".format(run_type.capitalize())))
