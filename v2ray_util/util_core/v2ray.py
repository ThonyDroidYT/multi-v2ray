...
def restart(port_open=False):
    """
    Reinicie el decorador v2ray después de ejecutar la función / método
    """  
    def decorate(func):
        @wraps(func)
        def wrapper(*args, **kw):
...
class V2ray:
    ...
    def docker_run(command, keyword):
        ...
        if V2ray.docker_status() or keyword == "stop":
            print(ColorStr.green("{} {} éxito !".format(run_type, keyword)))
        else:
            print(ColorStr.red("{} {} falló !".format(run_type, keyword)))
    ...
    def update(version=None):
        if is_ipv6(get_ip()):
            print(ColorStr.yellow(_("La red ipv6 no admite la actualización {soft} en línea, ¡descargue manualmente {soft} para actualizar!".format(soft=run_type))))
            if run_type == "xray":
                print(ColorStr.fuchsia(_("descargar Xray-linux-xx.zip ejecutar 'bash <(curl -L -s https://multi.netlify.app/go.sh) -l Xray-linux-xx.zip -x' para actualizar")))
            else:
                print(ColorStr.fuchsia(_("descargar v2ray-linux-xx.zip ejecutar 'bash <(curl -L -s https://multi.netlify.app/go.sh) -l v2ray-linux-xx.zip' para actualizar")))
            sys.exit(0)
        if os.path.exists("/.dockerenv"):
            V2ray.stop()
        #subprocess.Popen("curl -Ls https://multi.netlify.app/go.sh -o temp.sh", shell=True).wait()
        subprocess.Popen("curl -Ls https://v2ray.admplus.tk/go.sh -o temp.sh", shell=True).wait()
    ...
    def check(cls):
        ...
        if not os.path.exists("/usr/bin/{bin}/{bin}".format(bin=run_type)):
            print(ColorStr.yellow(_("check {soft} sin instalación, instalación automática {soft}..".format(soft=run_type))))
            cls.update()
            cls.new()
    ...
    def remove(cls):
        if os.path.exists("/.dockerenv"):
            print(ColorStr.yellow("Docker Run no admite eliminar {}!".format(run_type)))
            return
        cls.stop()
        ...
        print(ColorStr.green("{} Removido exitosamente.".format(run_type)))
        print(ColorStr.blue("Si es necesario, elimine el archivo de configuración y el archivo de registro manualmente."))
    ...
    def new(cls):
        ...
        new_uuid = uuid.uuid1()
        print("nuevo UUID: {}".format(ColorStr.green(str(new_uuid))))
        new_port = random_port(1000, 65535)
        print("nuevo puerto: {}".format(ColorStr.green(str(new_port))))
